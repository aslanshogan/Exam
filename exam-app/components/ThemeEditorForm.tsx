...
async function uploadFile(file: File, kind: string): Promise<string> {
  // Step 1: ask our server for a short-lived signed upload URL. This is
  // a tiny JSON request (filename + size only) — the file itself does
  // NOT pass through the Vercel function, so we avoid the ~4.5MB
  // FUNCTION_PAYLOAD_TOO_LARGE limit that blocks large videos.
  ...
  let sign: any = {};
  try { sign = JSON.parse(signText); } catch { sign = { error: signText || `Upload failed (HTTP ${signRes.status}).` }; }
  if (!signRes.ok) {
    if (signRes.status === 401) throw new Error("Session expired, please sign in again.");
    if (signRes.status === 403) throw new Error("You do not have permission to upload media.");
    throw new Error(sign.error || "Could not start upload.");
  }

  // Step 2: upload the file bytes DIRECTLY to Supabase Storage using the
  // signed URL. This goes browser → Supabase, not through Vercel, so the
  // real size limit is the app's own (e.g. 80MB for video), not 4.5MB.
  // The signed URL from createSignedUploadUrl is a PUT target; sending
  // the raw File as the body with its content type is what Storage
  // expects.
  ...
  if (!putRes.ok) {
    ...
    throw new Error(`Storage upload failed (HTTP ${putRes.status}). ${t.slice(0, 200)}`);
  }
