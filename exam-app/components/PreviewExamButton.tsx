...
export default function PreviewExamButton({ className = "" }: { className?: string }) {
  ...
  async function start() {
    ...
    if (!res.ok) {
      ...
      const msg = data.error || `Could not start preview (HTTP ${res.status}).`;
      if (/permission denied/i.test(msg)) {
        setError(
          "Database permission denied. Your SUPABASE_SERVICE_ROLE_KEY in Vercel is likely wrong (it must be the service_role key, not the anon key). Open /api/debug/env to confirm, then fix it and redeploy."
        );
        return;
      }
      setError(msg);
