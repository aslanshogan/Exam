...
are generated on **two free tiers**: **Tavily** does a real web search,
then **Google Gemini** writes questions grounded in those results.
Written source URLs are **required and server-enforced**: each URL is
verified against the URLs Tavily actually returned, and the admin import
route rejects (HTTP 400) any question lacking a valid written-source URL
— nothing is imported with empty sources. Video links are optional (real
YouTube/Vimeo video pages only; empty is fine). Requires two free env
vars (`TAVILY_API_KEY`, `GEMINI_API_KEY`) and one SQL run — full details
in **AI_TRAINER_SETUP.md**.
