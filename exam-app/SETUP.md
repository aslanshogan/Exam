...
| | v1 | v2 |
|---|---|---|
| Trainee login | Type your name, no account | Real account (email+password) **or** an access code |
...
4. Generate a random secret for access-code logins:
   ```bash
   openssl rand -hex 32
   ```
   Put it in `.env.local` as `ACCESS_CODE_SECRET`.

### Bootstrapping the first Super Admin
There's a chicken-and-egg problem: creating a user normally requires
already being a Super Admin. To create the very first one:
...
2. SQL Editor, run:
   ...
   ```
3. Log in at `/login` with that email/password. You're now a Super
   Admin and can create every other user from `/admin/users`.
...
Super Admin can:
- **Add a user** — give them a display name, pick a role, and either an
  email+password account OR an access code (or both).
...
- **Deactivate vs delete** — both are one-click buttons in the "Danger
  ...
  - **Delete User** — permanent. Removes their login, access code,
...
### Access codes vs email accounts
- **Email + password** → real Supabase Auth account, works for any role.
- **Access code** → lightweight, no email needed, **trainee-only**, and
  only grants access to `/exam` and `/result` (never `/admin`). Good for
  walk-up training sessions where creating real accounts isn't practical.
...
- [ ] Create one trainee with an access code instead of email — log in via the "Access Code" tab at `/login`
- [ ] **Repeat the access-code login test on your real Vercel deployment, not just `npm run dev`.** This exercises `middleware.ts` running on Vercel's actual Edge Runtime (local dev does not fully emulate Edge). On the live site: log in with the access code → open browser DevTools → Application/Storage → Cookies → confirm a `trainee_code_session` cookie was set → confirm `/exam` opens normally → manually navigate to `/admin` and confirm you're redirected to `/login` (code-session trainees must never reach `/admin`, even though the same cookie grants `/exam`/`/result`).
