    ...
    const flush = (rowNum: number) => {
      if (curQ && Object.keys(opts).length >= 2) {
        if (!opts["A"] || !opts["B"] || !correct) {
          problems.push(
            `[${sheetName}] row ~${rowNum}: incomplete question (missing A/B or correct answer) — "${curQ.slice(0, 60)}..."`
          );
        } else {
          parsed.push({
            category: sheetName.trim(),
            question: curQ.replace(NUM_PREFIX_RE, "").trim(),
            a: opts["A"] || "",
            b: opts["B"] || "",
            c: opts["C"] || "",
            d: opts["D"] || "",
            correct,
            explanation,
            sourceRowHint: `${sheetName}!row${rowNum}`,
          });
        }
      }
      curQ = null;
