...
export default function AiGeneratorPage() {
  ...
  async function generateAndSave() {
    if (!chosenTopic) { setError("Choose a topic (or type a custom one)."); return; }
    if (!directCategory) { setError("Pick a bank category to save into."); return; }
    ...
    while (remaining > 0) {
      ...
      const { ok, data, error } = await apiFetch("/api/admin/ai-generator", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ category: chosenTopic, difficulty, count: ask, save: true, category_id: directCategory, requireVideo }),
      });
      if (!ok || !data) {
  ...
  async function importSelected() {
    ...
    if (picked.some((q) => !q.category_id)) { setError("Every selected question needs a category."); return; }
    ...
    setMessage(`Imported ${data.imported} question(s) with their sources into the question bank (Admin → Questions).`);
    setResults([]);
  }
        ...
        <div className="flex-1 space-y-6">
          ...
          <div className="card p-5 space-y-3 max-w-2xl">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              <select value={directCategory} onChange={(e) => setDirectCategory(e.target.value)} className="border rounded-lg px-2 py-2 text-sm" title="Save into this bank category">
                <option value="">— bank category —</option>
                {categories.map((c) => <option key={c.id} value={c.id}>{c.name}</option>)}
              </select>
              <button
                onClick={generateAndSave}
                disabled={generating || savingDirect || !directCategory}
                className="bg-brandGreen text-navy-900 font-bold px-5 py-2.5 rounded-lg disabled:opacity-60"
                title={!directCategory ? "Pick a bank category first" : "Generate and save straight into the question bank"}
              >
            ...
            <p className="text-[11px] text-gray-400">
              "Generate for Review" lets you edit before importing. "Generate &amp; save to bank" skips review and writes
              questions (with their verified sources + video links) straight into the chosen category.
            </p>
            {count >= 20 && !generating && (
            ...
            <>
              ...
              <div className="card p-3 flex flex-wrap items-end gap-3">
                <div>
                  ...
                  <select value={fCategory} onChange={(e) => setFCategory(e.target.value)} className="border rounded-lg px-2 py-1 text-xs">
                  ...
                  <div key={i} className={"card p-5 space-y-3 " + (q.include ? "" : "opacity-50")}>
                    <div className="flex items-center justify-between gap-3">
                      ...
                      <div className="flex items-center gap-2">
                        <select value={q.category_id} onChange={(e) => upd(i, { category_id: e.target.value })} className="border rounded-lg px-2 py-1 text-xs" title="Bank category">
