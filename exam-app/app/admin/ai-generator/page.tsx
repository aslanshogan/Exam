  ...
  async function generate() {
    ...
    while (remaining > 0) {
      ...
      const { ok, data, error } = await apiFetch("/api/admin/ai-generator", {
        ...
        body: JSON.stringify({ category: chosenTopic, difficulty, count: ask }),
      });
      if (!ok || !data) {
        setError(
      ...
      <div className="flex-1 max-w-7xl mx-auto w-full px-4 sm:px-6 py-8 flex gap-8">
        ...
        <div className="flex-1 space-y-6">
          ...
          <div className="card p-5 space-y-3 max-w-2xl">
            <div>
              ...
              <select value={topic} onChange={(e) => setTopic(e.target.value)} className="border rounded-lg px-3 py-2 w-full">
                ...
                <optgroup label="Technical topics">
                  {TRAINER_TOPICS.map((t) => <option key={t} value={t}>{t}</option>)}
                </optgroup>
            ...
            <button onClick={generate} disabled={generating} className="bg-navy-900 text-white font-bold px-5 py-2.5 rounded-lg disabled:opacity-60">
