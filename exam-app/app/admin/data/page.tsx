...
export default function AdminDataPage() {
  ...
  async function load() {
    const { ok, data } = await apiFetch("/api/admin/data/counts");
  ...
  async function handleClearResults() {
    ...
    setClearing(true);
    setMessage(null);
    const { ok, data, error } = await apiFetch("/api/admin/data/clear-results", {
      method: "POST",
    ...
    setClearing(false);
    if (!ok || !data) {
      setMessage(error || "Could not clear results.");
      return;
    }
