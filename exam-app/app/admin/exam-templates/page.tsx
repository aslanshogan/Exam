...
export default function AdminExamTemplatesPage() {
  ...
  const [openQuestionsFor, setOpenQuestionsFor] = useState<string | null>(null);
  ...
  async function load() {
    const [tRes, uRes] = await Promise.all([
      apiFetch("/api/admin/exam-templates"),
      apiFetch("/api/admin/users"),
    ]);
    if (!tRes.ok) { setMessage(tRes.error || "Could not load templates."); return; }
    setTemplates(tRes.data?.templates || []);
    setTrainees(((uRes.data?.users) || []).filter((u: any) => u.role_id === "trainee"));
  }
  useEffect(() => { load(); }, []);
  ...
  async function handleCreate() {
    ...
    setCreating(true);
    setMessage(null);
    const { ok, data, error } = await apiFetch("/api/admin/exam-templates", {
      method: "POST",
    ...
    setCreating(false);
    if (!ok || !data) {
      setMessage(error || "Could not create exam template.");
      return;
    }
    setNewName("");
    setShuffleNew(false);
  ...
  async function viewQuestions(templateId: string) {
    if (openQuestionsFor === templateId) {
      setOpenQuestionsFor(null);
      return;
    }
    setOpenAssignFor(null);
    setOpenQuestionsFor(templateId);
    setLoadingQuestions(true);
    setTemplateQuestions([]);
    const { ok, data, error } = await apiFetch(`/api/admin/exam-templates/${templateId}`);
    setLoadingQuestions(false);
    if (!ok || !data) { setMessage(error || "Could not load template questions."); return; }
  ...
  async function saveAssignment(templateId: string) {
    ...
    if (toAssign.length > 0) {
      const r = await apiFetch(`/api/admin/exam-templates/${templateId}/assign`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ userIds: toAssign, action: "assign" }),
      });
      if (!r.ok) { setMessage(r.error || "Could not assign template."); return; }
    }
    if (toUnassign.length > 0) {
      ...
      if (!r.ok) { setMessage(r.error || "Could not update assignment."); return; }
    }
  ...
  async function handleDelete(templateId: string, name: string) {
    ...
    const { ok, error } = await apiFetch(`/api/admin/exam-templates/${templateId}`, { method: "DELETE" });
    if (!ok) { setMessage(error || "Could not delete template."); return; }
    load();
  }
                  ...
                  <Fragment key={t.id}>
                    <tr className={i % 2 === 0 ? "bg-white" : "bg-gray-50"}>
                      ...
                      <td className="px-4 py-3 space-x-3 whitespace-nowrap">
                        <button onClick={() => viewQuestions(t.id)} className="text-navy-900 hover:underline">
                          {openQuestionsFor === t.id ? "Hide" : "View Questions"}
                        </button>
                    ...
                    {openQuestionsFor === t.id && (
                      <tr>
                        <td colSpan={5} className="px-4 py-4 bg-gray-50 border-t">
                          {loadingQuestions ? (
                            <p className="text-sm text-gray-400">Loading questions...</p>
                          ) : templateQuestions.length === 0 ? (
                            <p className="text-sm text-gray-400">This template has no questions.</p>
                          ) : (
                            <ol className="space-y-3 list-decimal list-inside">
                              {templateQuestions.map((q) => (
                                <li key={q.question_number} className="text-sm">
                                  <span className="font-medium text-navy-900">{q.question_text}</span>
                                  <span className="ml-2 text-xs text-gray-400">[{q.category_name}]</span>
                                  <ul className="mt-1 ml-5 space-y-0.5 text-xs">
                                    {(["A", "B", "C", "D"] as const).map((L) => {
                                      ...
                                      return (
                                        <li key={L} className={correct ? "text-brandGreen-700 font-semibold" : "text-gray-600"}>
                                          {correct ? "✓ " : ""}{L}. {val}
                                        </li>
                                      );
                                    })}
