...
export default function AdminImportPage() {
  ...
  async function handleImport() {
    ...
    let res: Response;
    try {
      res = await fetch("/api/admin/import", { method: "POST", body: formData });
    } catch (e: any) {
      setLoading(false);
      setError(`Network error during import: ${e?.message || e}`);
      return;
    }
    ...
    try {
      ...
      setLoading(false);
      setError(`Import failed (HTTP ${res.status}). Server response: ${text.slice(0, 300)}`);
      return;
    }
    ...
    if (!res.ok) {
      ...
      if (res.status === 403) { setError("You do not have permission to import questions."); return; }
      // 422 (0 valid questions) still carries a useful report — show both.
      setError(data.error || `Import failed (HTTP ${res.status}).`);
      if (data.problems || data.totalParsed !== undefined) setReport(data);
      return;
    }
    setReport(data);
  }
