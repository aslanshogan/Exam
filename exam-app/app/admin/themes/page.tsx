...
export default function AdminThemesPage() {
  ...
  useEffect(() => {
    apiFetch("/api/admin/themes/default").then(({ ok, data }) => {
      if (!ok || !data) return;
      const t = data.theme;
      setDefaultTheme(
        t
    ...
    apiFetch("/api/admin/users").then(({ ok, data }) => { if (ok && data) setUsers(data.users || []); });
    apiFetch("/api/admin/settings").then(({ ok, data }) => { if (ok && data) setMusicGloballyEnabled(data.settings?.music_globally_enabled ?? true); });
  }, []);

  async function toggleGlobalMusic() {
    setSavingSetting(true);
    const next = !musicGloballyEnabled;
    const { ok } = await apiFetch("/api/admin/settings", {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ music_globally_enabled: next }),
    });
    if (ok) setMusicGloballyEnabled(next);
