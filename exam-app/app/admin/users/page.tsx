...
export default function AdminUsersPage() {
  ...
  async function load() {
    const { ok, data, error } = await apiFetch("/api/admin/users");
    if (ok && data) {
      setUsers(data.users || []);
      setCurrentUserId(data.currentUserId || null);
      setError(null);
    } else {
      setError(error || "Could not load users.");
    }
  }
  useEffect(() => { load(); }, []);
  ...
  async function handleBlock(u: User) {
    ...
    setMessage(null);
    const { ok, error } = await apiFetch(`/api/admin/users/${u.id}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ is_active: false }),
    });
    if (!ok) { setError(error || "Could not block user."); return; }
    setError(null);
    load();
  }
  ...
  async function handleActivate(u: User) {
    ...
    if (!ok) { setError(error || "Could not activate user."); return; }
    setError(null);
    load();
  }
  ...
  async function handleDelete(u: User) {
    if (
      ...
      return;
    setMessage(null);
    const { ok, error } = await apiFetch(`/api/admin/users/${u.id}`, { method: "DELETE" });
    if (!ok) { setError(error || "Could not delete user."); return; }
    setError(null);
