...
function AdminUserDetailPage() {
  ...
  const [profileForm, setProfileForm] = useState({ full_name: "", username: "", role_id: "trainee", is_active: true });
  const [accessForm, setAccessForm] = useState({ allowed_to_take: true, allow_retake: false, max_attempts: 1 });
  const [message, setMessage] = useState<string | null>(null);
  const [saving, setSaving] = useState(false);
  const [deleting, setDeleting] = useState(false);

  async function load() {
    const { ok, data: d, error } = await apiFetch(`/api/admin/users/${userId}`);
    if (!ok || !d) { setMessage(error || "Could not load this user."); return; }
    setData(d);
    if (d.profile) {
      setProfileForm({
        full_name: d.profile.full_name || d.profile.display_name || "",
        username: d.profile.username || "",
        role_id: d.profile.role_id,
  ...
  async function saveProfile() {
    ...
    const { ok, error } = await apiFetch(`/api/admin/users/${userId}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(body),
    });
    setSaving(false);
    setMessage(ok ? "Saved." : error || "Save failed.");
    load();
  }
  ...
  async function handleQuickDeactivate() {
    const name = data.profile.full_name || data.profile.display_name;
    if (!confirm(`Deactivate ${name}? They'll immediately be unable to log in or take the exam. This can be reversed any time by checking "Active" again.`)) {
      return;
    }
    setSaving(true);
    setMessage(null);
    ...
    setSaving(false);
    setMessage(ok ? "Deactivated." : error || "Could not deactivate.");
    load();
  }
  ...
  async function handleDelete() {
    ...
    const name = data.profile.full_name || data.profile.display_name;
    const warning =
      `This PERMANENTLY deletes ${name}'s account — their username login, theme, and exam-access settings.\n\n` +
      (attemptCount > 0
        ? `They have ${attemptCount} exam attempt(s) on record — those results are NOT deleted and will still show under the name "${name}" on the Results pages, but will no longer be linked to an account.\n\n`
    ...
    setDeleting(true);
    setMessage(null);
    const { ok, error } = await apiFetch(`/api/admin/users/${userId}?hard=true`, { method: "DELETE" });
    setDeleting(false);
    if (!ok) {
      setMessage(error || "Could not delete this user.");
      return;
    }
        ...
        <div className="flex-1 space-y-6">
          ...
          {tab === "profile" && (
            <div className="card p-5 space-y-3 max-w-md">
              ...
              <div>
                <label className="block text-xs text-gray-500 mb-1">Username (their login — no password exists)</label>
                <input
              ...
              <label className="flex items-center gap-2 text-sm">
                ...
                Active (blocked users cannot log in or take the exam)
              </label>
