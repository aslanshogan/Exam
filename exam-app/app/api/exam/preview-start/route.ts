...
export async function POST(req: NextRequest) {
  ...
  // Try to create the attempt WITH the is_preview flag. If the column
  // doesn't exist yet (schema not re-run after upgrading), fall back to
  // inserting without it so preview still works — the attempt just won't
  // be flagged until the schema is updated.
  ...
  let attemptErr: any = null;
  {
    const res = await supabase
      ...
      .single();
    attempt = res.data;
    attemptErr = res.error;
  }
  if (attemptErr && /is_preview/.test(attemptErr.message || "")) {
    console.warn("[preview-start] is_preview column missing — falling back. Re-run schema.sql to enable preview labelling.");
    const res = await supabase
      .from("exam_attempts")
      .insert({ profile_id: profile.id, trainee_name: `${profile.display_name} (Admin Preview)`, status: "in_progress" })
      .select("id")
      .single();
    attempt = res.data;
    attemptErr = res.error;
  ...
  if (attemptErr || !attempt) {
    console.error("[preview-start] could not create attempt:", attemptErr);
  ...
  if (eaqErr) {
    console.error("[preview-start] could not insert questions:", eaqErr);
