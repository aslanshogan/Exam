...
export async function POST(req: NextRequest) {
  ...
  if (unanswered.length > 0) {
    const unansweredNumbers = unanswered.map((r) => r.question_number).sort((a, b) => a - b);
    return NextResponse.json(
      {
        error: `${unanswered.length} question(s) still unanswered.`,
  ...
  // Count this attempt against the trainee's allowance — but NOT for
  // admin preview attempts (is_preview = true), which must not consume
  // anyone's real allowance. Tolerate the column not existing yet.
  let attemptRow: { profile_id: string | null; is_preview?: boolean } | null = null;
  {
    const res = await supabase
      .from("exam_attempts")
      .select("profile_id, is_preview")
      .eq("id", attemptId)
      .single();
    if (res.error && /is_preview/.test(res.error.message || "")) {
      const res2 = await supabase.from("exam_attempts").select("profile_id").eq("id", attemptId).single();
      attemptRow = res2.data as any;
    } else {
      attemptRow = res.data as any;
    }
  }
  if (attemptRow?.profile_id && !attemptRow.is_preview) {
