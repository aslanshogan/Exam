...
export async function POST(req: NextRequest) {
  ...
  let categoryId = typeof body.category_id === "string" ? body.category_id : "";
  if (!category) return NextResponse.json({ error: "category is required." }, { status: 400 });

  // When saving directly, if no bank category was chosen, auto-create /
  // reuse one named after the topic — so questions always have a home and
  // show up in Admin → Questions without the admin setting it up first.
  if (save && !categoryId) {
    const ensured = await ensureCategoryByName(category);
    if (!ensured.id) {
      return NextResponse.json({ error: "Could not create a bank category for this topic." }, { status: 500 });
    }
