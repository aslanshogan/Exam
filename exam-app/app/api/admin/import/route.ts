...
function parseWorkbook(buf: ArrayBuffer) {
  ...
  for (const sheetName of wb.SheetNames) {
    ...
    // sheet_to_json can yield SPARSE arrays (holes where cells are
    // blank/merged). Array.prototype.map preserves holes, and for...of
    // over a hole yields `undefined` — which then crashes on cell.match.
    // Array.from({length}) densifies every row so every cell is a real
    // string, never a hole.
    const rows = raw.map((r) => {
      const src = r || [];
      const len = src.length;
      return Array.from({ length: len }, (_, i) => cleanCell((src[i] ?? "").toString()));
    });
