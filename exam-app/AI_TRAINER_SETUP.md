...
`TAVILY_API_KEY` and `GROQ_API_KEY` (tick Production) → **Redeploy**.

Optional: `GROQ_MODEL` (default `llama-3.3-70b-versatile`).

Cost: both run on free tiers — no payment method required to start.
Watch the free quotas (Tavily ~1,000 searches/month; Groq has a
generous daily limit). Each generated question uses one Tavily search
and one Groq call. If a key is missing, the trainer/generator show a
