...
// Groq — OpenAI-compatible chat completions, free tier, no billing.
...
async function callOnce(opts: {
  ...
  if (!groqKey) {
    return {
      questions: [],
      error:
        "AI is not configured. Add GROQ_API_KEY to your environment variables (free key at console.groq.com), then redeploy.",
    };
  }
  ...
  // STEP 2 — Groq writes the questions grounded in those results.
  ...
  if (!res.ok) {
    console.error("[aiTrainer] Groq error:", res.status, bodyText.slice(0, 500));
  ...
  // Groq (OpenAI-compatible) returns choices[0].message.content as text.
  let text = "";
  try {
