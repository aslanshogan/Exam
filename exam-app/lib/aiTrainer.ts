 ...
 *   2. GROQ (writing) — GROQ_API_KEY. Writes the questions
...
/**
 * AI providers for writing questions, tried IN ORDER until one succeeds.
 * All use the OpenAI-compatible chat/completions format, so one code path
 * handles them. Add whichever free keys you have in Vercel — the app uses
 * the first provider whose key is set and whose quota isn't exhausted,
 * automatically falling through to the next on quota/rate-limit/errors.
 * Only providers with a key present are attempted.
 */
type AIProvider = { name: string; url: string; key: string | undefined; model: string };
function aiProviders(): AIProvider[] {
  return [
    ...
    { name: "Cerebras", url: "https://api.cerebras.ai/v1/chat/completions", key: process.env.CEREBRAS_API_KEY, model: process.env.CEREBRAS_MODEL || "llama-3.3-70b" },
    { name: "Together", url: "https://api.together.xyz/v1/chat/completions", key: process.env.TOGETHER_API_KEY, model: process.env.TOGETHER_MODEL || "meta-llama/Llama-3.3-70B-Instruct-Turbo-Free" },
  ].filter((p) => !!p.key);
}
...
You are given REAL web search results about "${category}". Write ${count} American-style multiple-choice question${count > 1 ? "s" : ""} that are SPECIFICALLY about "${category}", grounded ONLY in what these results say.
${difficultyLine}
...
RELEVANCE (critical):
- Every question MUST be specifically about "${category}" — its components, operation, faults, control, protection, or maintenance.
- Do NOT write vague identification questions like "What type of turbine is this?", "Which component is shown?", or "What is this system?" — the reader has no picture, so these are unanswerable and off-topic.
- Do NOT write a question that could apply to almost any equipment ("What is the main purpose of maintenance?"). Each question must test a real, specific fact about "${category}".
- If the search results are too generic to write a specific question, write about the closest specific aspect of "${category}" they do mention — never a generic filler question.

ANSWER ACCURACY (critical):
- Decide the correct answer by reasoning from the search results, then re-check it before finalizing. The letter in "correct_answer" MUST be the actually-correct option.
- The "explanation" must clearly justify why the marked answer is correct; if you can't justify it from the material, pick a different question you can get right.
- The three wrong options must be genuinely incorrect (not also-correct), but still believable to a knowledgeable operator.

OTHER RULES:
- Exactly 4 options (A–D), exactly one correct; vary which letter is correct across the set.
- "explanation": 2–4 sentences teaching WHY the correct answer is right and why the others are wrong.
- "subcategory": a short (1–4 word) specific sub-topic of "${category}".
- "sources": 1–3 items as {"title":"...","url":"..."}. Each URL MUST be copied EXACTLY from a "URL:" line in the SEARCH RESULTS above. NEVER invent, guess, shorten, or modify a URL, and never cite a URL not in the list. Every question needs at least one.
...
function validateQuestion(raw: any, requestedDifficulty: string, allowed: Set<string> | null, topic?: string): GeneratedQuestion | null {
  ...
  // Reject vague identification / generic questions that aren't tied to
  // the specific topic (the "what turbine is this" class the user flagged).
  const genericPatterns = [
    /what (type|kind) of \w+ is (this|shown|depicted)/i,
    /which (component|part|system|device) is (this|shown|depicted)/i,
    /what is (this|the) (component|part|system|device|equipment)\??$/i,
    /identify the (component|part|shown)/i,
    /in the (image|figure|picture|diagram)/i,
  ];
  if (genericPatterns.some((re) => re.test(qtext))) return null;

  // Topic relevance: the question/subcategory/explanation should share a
  // meaningful word with the requested topic. Guards against off-topic
  // drift when the search results were broad.
  if (topic) {
    const stop = new Set(["the","a","an","of","and","or","for","to","in","on","is","are","with","system","systems","general","basics","power","plant"]);
    const topicWords = normalizeQuestionText(topic).split(" ").filter((w) => w.length >= 3 && !stop.has(w));
    if (topicWords.length > 0) {
      const hay = normalizeQuestionText(`${qtext} ${raw.subcategory || ""} ${raw.explanation || ""} ${raw.answer_a} ${raw.answer_b} ${raw.answer_c} ${raw.answer_d}`);
      const hit = topicWords.some((w) => hay.includes(w));
      if (!hit) return null; // no connection to the topic → drop it
    }
  }
...
async function callOnce(opts: {
  ...
  if (providers.length === 0) {
    return {
      questions: [],
      error:
        "AI is not configured. Add at least one AI key (GROQ_API_KEY — free at console.groq.com; optionally OPENROUTER_API_KEY, CEREBRAS_API_KEY, TOGETHER_API_KEY as backups), then redeploy.",
    };
  }

  // STEP 1 — real web search (Tavily). Two focused queries about the
  // exact topic give more on-topic material than one generic query, so
  // the model writes questions about THIS topic (not vague "what turbine
  // is this" ones). Results + their URLs form the allow-set.
  const s1 = await tavilySearch(`${opts.category} components operation working principle`);
  const s2 = await tavilySearch(`${opts.category} faults troubleshooting maintenance operator`);
  if (s1.error && s2.error) return { questions: [], error: s1.error };
  const mergedResults = [...s1.results, ...s2.results].slice(0, 12);
  const search = {
    results: mergedResults,
    allowed: new Set<string>([...s1.allowed, ...s2.allowed]),
  };
  ...
  // STEP 2 — an AI writes the questions grounded in those results. Try
  // each configured provider in order; on quota/rate-limit/error, fall
  // through to the next. This is the "one token runs out → use another"
  // behavior.
  ...
  for (const provider of providers) {
    ...
    try {
      res = await fetch(provider.url, {
        ...
        body: JSON.stringify({
          ...
          temperature: 0.6,
          max_tokens: Math.min(8000, 1200 + opts.count * 600),
          response_format: { type: "json_object" },
          messages: [
            {
              role: "system",
              content:
                "You write technical exam questions. Respond with ONLY valid JSON — an object with a \"questions\" array — and nothing else.",
            },
      ...
      providerErrors.push(`${provider.name}: network error`);
      continue; // try next provider
    }

    const bodyText = await res.text();
    if (!res.ok) {
      // 429 (rate limit / quota) and 402 (out of credits) → fall through.
      console.error(`[aiTrainer] ${provider.name} error:`, res.status, bodyText.slice(0, 300));
    ...
    try {
      ...
      providerErrors.push(`${provider.name}: unreadable response`);
      continue;
    }
    if (text.trim()) break; // got a usable answer — stop trying providers
    providerErrors.push(`${provider.name}: empty response`);
  }

  if (!text.trim()) {
    return {
      questions: [],
      error: `All AI providers failed or are out of quota. Details: ${providerErrors.join(" | ")}`,
    };
  }
  ...
  // and each question is checked for relevance to the requested topic.
  const questions = arr
    .map((raw) => validateQuestion(raw, opts.difficulty, search.allowed, opts.category))
