...
- "video_sources": ONLY if one of the SEARCH RESULTS above is a real YouTube or Vimeo VIDEO page. Then {"title","url","platform","reason"} with the URL copied exactly. Otherwise []. Never invent a video link or use a channel/search page.
...
async function callOnce(opts: {
  ...
  const search = await tavilySearch(
    `${opts.category} power plant operation engineering explained`
  );
